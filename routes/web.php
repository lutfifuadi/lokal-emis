...
use App\Http\Controllers\Admin\MasterSekolahController;
...
use App\Http\Controllers\Admin\MasterSiswaController;
use App\Http\Controllers\Sekolah\DashboardController as SekolahDashboardController;
use App\Http\Controllers\Sekolah\ApprovalController;
...
Route::middleware(['auth'])->get('/', function () {
    ...
    return match(true) {
        in_array($role, ['Super Admin', 'Dinas', 'Operator']) => redirect()->route('admin.dashboard'),
        in_array($role, ['Kepala Sekolah', 'Guru']) => redirect()->route('sekolah.dashboard'),
        in_array($role, ['Siswa', 'Orang Tua']) => redirect()->route('siswa.dashboard'),
        default => redirect()->route('login'),
    };
})->name('dashboard');

// ==================== ADMIN PORTAL ====================
Route::middleware(['auth', 'role:Super Admin|Dinas|Operator'])
    ->prefix('admin')
    ->name('admin.')
    ->group(function () {
        ...
        Route::prefix('master')->name('master.')->group(function () {
            ...
            Route::get('/tahun-ajaran', [MasterTahunAjaranController::class, 'index'])->name('tahun-ajaran');
            Route::get('/kelas', [MasterKelasController::class, 'index'])->name('kelas');
            Route::get('/users', [MasterUserController::class, 'index'])->name('users');
            Route::get('/siswa', [MasterSiswaController::class, 'index'])->name('siswa');
            Route::get('/siswa/tambah', [MasterSiswaController::class, 'tambah'])->name('siswa.tambah');
            Route::get('/siswa/{id}/edit', [MasterSiswaController::class, 'edit'])->name('siswa.edit');

            // Import routes
            Route::post('/import', [MasterImportController::class, 'import'])->name('import');
            Route::get('/import/sample/{entity}', [MasterImportController::class, 'downloadSample'])->name('import.sample');
            ...
            Route::prefix('google-sheet-settings')->name('google-sheet-settings.')->group(function () {
                Route::get('/', [GoogleSheetSettingController::class, 'index'])->name('index');
                Route::get('/create', [GoogleSheetSettingController::class, 'create'])->name('create');
                Route::post('/', [GoogleSheetSettingController::class, 'store'])->name('store');
                Route::get('/{id}/edit', [GoogleSheetSettingController::class, 'edit'])->name('edit');
                Route::put('/{id}', [GoogleSheetSettingController::class, 'update'])->name('update');
                Route::delete('/{id}', [GoogleSheetSettingController::class, 'destroy'])->name('destroy');
                Route::post('/{id}/test-connection', [GoogleSheetSettingController::class, 'testConnection'])->name('test-connection');
            });
            Route::post('/google-sheet-sync/{entity}', [GoogleSheetSettingController::class, 'sync'])->name('google-sheet-sync');
        });
        Route::post('/impersonate/{id}', [ImpersonateController::class, 'start'])->name('impersonate.start')->whereNumber('id');
    });


// Route stop impersonation — di luar grup admin karena user mungkin di portal lain saat impersonation aktif
Route::middleware(['auth'])->match(['get', 'post'], '/admin/impersonate/stop', [ImpersonateController::class, 'stop'])->name('impersonate.stop');

// ==================== SEKOLAH PORTAL ====================
Route::middleware(['auth', 'role:Kepala Sekolah|Guru'])
    ->prefix('sekolah')
    ->name('sekolah.')
    ->group(function () {
        Route::get('/', [SekolahDashboardController::class, 'index'])->name('dashboard');
        Route::prefix('approval')->name('approval.')->group(function () {
            Route::get('/antrian', [ApprovalController::class, 'antrian'])->name('antrian');
        });
    });

// ==================== SISWA PORTAL ====================
Route::middleware(['auth', 'role:Siswa|Orang Tua'])
    ->prefix('siswa')
    ->name('siswa.')
    ->group(function () {
        Route::get('/', [SiswaDashboardController::class, 'index'])->name('dashboard');
        Route::get('/profil', [ProfilController::class, 'profil'])->name('profil');
        Route::get('/perubahan', [ProfilController::class, 'perubahan'])->name('perubahan');
    });
